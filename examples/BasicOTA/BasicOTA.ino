 ...
 * - Automatic firmware rollback protection
 *
 * Usage:
 ...
 * 4. After saving, reconnect the device to your WiFi network
...
void setup() {
    ...
    if (!ota.isProvisioned()) {
        ...
        bool provisioned = ota.startProvisioningPortal("SafeGithubOTA-Setup");

        if (provisioned) {
            Serial.println("Provisioning complete!");
        } else {
            Serial.println("Provisioning timed out or failed.");
        }
    }
    ...
    // This also handles rollback validation on first boot after OTA.
    // Since no validation callback is set, new firmware is auto-confirmed.
    SGO_Error err = ota.begin();
    if (err == SGO_Error::NOT_PROVISIONED) {
